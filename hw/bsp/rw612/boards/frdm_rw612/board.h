 ...
 *
 * Modified for TinyUSB integration while preserving NXP's critical
 * RAM-resident clock configuration functions (AT_QUICKACCESS_SECTION_CODE).
 */
...
/*******************************************************************************
 * TinyUSB-specific definitions
 ******************************************************************************/
// LED - Blue LED on GPIO Port 0, Pin 0
#define LED_GPIO              GPIO
#define LED_CLK               kCLOCK_HsGpio0
#define LED_PIN               0
#define LED_STATE_ON          0  // Active low

// SW2 button on GPIO Port 0, Pin 11
#define BUTTON_GPIO           GPIO
#define BUTTON_CLK            kCLOCK_HsGpio0
#define BUTTON_PIN            11
#define BUTTON_STATE_ACTIVE   0  // Active low

// UART - FLEXCOMM3 with USB bridge support
#define UART_DEV                   USART3
#define UART_FLEXCOMM_INST         3

#include "fsl_clock.h"

static inline void board_uart_init_clock(void) {
  /* Attach FRG to FLEXCOMM3 (UART) */
  const clock_frg_clk_config_t frgClkConfig = {3, kCLOCK_FrgPllDiv, 255, 0};
  CLOCK_SetFRGClock(&frgClkConfig);
  CLOCK_AttachClk(kFRG_to_FLEXCOMM3);
  RESET_ClearPeripheralReset(kFC3_RST_SHIFT_RSTn);
}

/*******************************************************************************
 * NXP SDK standard definitions
 ******************************************************************************/
...
/* CRITICAL: These functions MUST run from RAM when XIP is enabled.
 * AT_QUICKACCESS_SECTION_CODE places them in the "CodeQuickAccess" section
 * which is linked to RAM by the linker script. This prevents hard faults
 * when modifying FlexSPI clocks while executing from flash.
 */
